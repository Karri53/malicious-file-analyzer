...
export default function URLAnalyzer() {
  ...
  const inputBg     = isDark ? '#181510'  : '#F5EFE5'
  const inputText   = isDark ? '#EEE8D5'  : '#1A1508'
  ...
  return (
    <div style={{ padding: '56px 120px 80px', background: bg, minHeight: '100vh', position: 'relative', zIndex: 0 }}>
      ...
      <p style={{ fontSize: '15px', color: textMuted, fontWeight: '300', maxWidth: '520px', lineHeight: '1.6', marginBottom: '40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        {/* Left: input panel */}
        <div style={{ background: surface, border: `1px solid ${pageState === 'error' ? 'rgba(224,85,85,0.3)' : border}`, borderRadius: '14px', padding: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
            <span style={{ fontFamily: 'Space Mono', fontSize: '10px', color: textFaint, letterSpacing: '0.14em' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '14px' }}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${pageState === 'error' ? 'rgba(224,85,85,0.5)' : border}`,
                borderRadius: '8px', padding: '12px 16px',
                fontFamily: 'Space Mono', fontSize: '12px',
                color: pageState === 'error' ? '#E05555' : inputText,
                outline: 'none',
              }}
            />
            <button
              ...
              style={{
                ...
                background: pageState === 'error' ? 'rgba(224,85,85,0.12)' : pageState === 'processing' ? border : '#D0BC77',
                color: pageState === 'error' ? '#E05555' : pageState === 'processing' ? textFaint : '#0A0906',
              }}
            >
          ...
          {pageState === 'processing' && (
            <div>
              ...
              {processingSteps.map((step, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 0', borderBottom: i < processingSteps.length - 1 ? `1px solid ${borderDim}` : 'none' }}>
          ...
          {pageState === 'error' && (
            <div>
              <div style={{ background: 'rgba(224,85,85,0.06)', border: '1px solid rgba(224,85,85,0.2)', borderRadius: '8px', padding: '14px 16px', marginBottom: '12px' }}>
                ...
                <div style={{ fontSize: '12px', color: textMuted, fontWeight: '300', lineHeight: '1.55' }}>We couldn't fetch this URL in our sandbox. The server may be down, the URL is malformed, or access is restricted.</div>
              </div>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: '8px', fontSize: '12px', color: textMuted, marginBottom: '6px', fontWeight: '300' }}>
                  <span style={{ color: '#E05555' }}>→</span>
                  <span><span style={{ color: textPrimary, fontWeight: '500' }}>{bold}</span>{rest}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: '14px', padding: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
            <span style={{ fontFamily: 'Space Mono', fontSize: '10px', color: textFaint, letterSpacing: '0.14em' }}>EXAMPLE SCAN HISTORY</span>
            <div style={{ flex: 1, height: '1px', background: border }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {exampleHistory.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: surface2, borderRadius: '8px', padding: '12px 16px' }}>
                <span style={{ fontFamily: 'Space Mono', fontSize: '11px', color: textMuted }}>{item.url}</span>
